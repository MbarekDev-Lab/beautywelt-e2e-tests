...
test.describe('Checkout', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.request.post('https://www.beautywelt.de/bestellvorgang.php?editRechnungsadresse=1', {
  ...
  test('should redirect to cart if cart is empty', async ({ page }) => {
    ...
    await page.waitForURL('/warenkorb.php');
  });
  ...
  test('should display checkout form', async ({ page }) => {
    ...
    await expect(page.locator('.bwkb .bwjz')).toBeVisible();
  ...
  test('should complete checkout successfully', async ({ page }) => {
    ...
    await page.locator('#firstName').fill('test-bestellung');
    await page.locator('#lastName').fill('tester');
    await page.locator('#address').fill('123 hauptstrasse');
    await page.locator('#city').fill('Buchholz');
    await page.locator('#state').selectOption('Niedersachsen');
    await page.locator('#zip').fill('21234');
    await page.locator('#phone').fill('123-456-7890');

    // Fill payment information
    await page.locator('#cardName').fill('test bestellung');
    await page.locator('#cardNumber').fill('4032037094157837');
    await page.locator('#expiry').fill('07/2030');
    await page.locator('#cvv').fill('593');
