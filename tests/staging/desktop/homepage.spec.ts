...
async function assertSameOriginDestination( link: Locator, baseURL: string, ): Promise<void> {
...
test.describe('Desktop homepage @staging @requires-authorization', () => {
    ...
    test('loads the desktop homepage with its primary landmarks', async ({ page, homePage, }): Promise<void> => {
    ...
    test('exposes every primary category destination in the desktop navigation', async ({ homePage, }): Promise<void> => {
    ...
    test('submits a homepage search and renders the search results page', async ({ page, homePage, }): Promise<void> => {
    ...
    test('renders linked hero and promotional imagery with safe destinations', async ({ baseURL, homePage, }): Promise<void> => {
        if (!baseURL) {
    ...
    test('renders complete product carousel cards', async ({ homePage, }): Promise<void> => {
        const productCards = await homePage.getVisibleProductCards();

        expect( productCards.length, 'Expected at least one visible product card on the homepage.', ).toBeGreaterThan(0);
    ...
    test('renders brand promotions and the newsletter signup section', async ({ baseURL, homePage,}): Promise<void> => {
        if (!baseURL) {
        ...
        expect( visibleBrandLinks.length, 'Expected at least one visible brand promotion.', ).toBeGreaterThan(0);
