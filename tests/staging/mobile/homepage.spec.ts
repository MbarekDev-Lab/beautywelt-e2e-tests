...
function assertSuccessfulResponse(response: Response | null,): asserts response is Response {
...
async function findFirstExistingLocator(candidates: Locator[], description: string,): Promise<Locator> {
    ...
    throw new Error(
        `Could not find ${description}. Current URL: ${candidates[0]?.page().url() ?? 'unknown'
        }`,
    );
}
...
async function selectSalutationIfPresent(page: Page): Promise<void> {
    ...
    const options = await salutationSelect
        .locator('option')
        .allTextContents();

    const herrOption = options.find((option) =>
        /^Herr$/i.test(option.trim()),
    );
...
async function fillRequiredInput(locator: Locator, value: string, fieldName: string,): Promise<void> {
    await expect(locator, `Expected the ${fieldName} input to be available on checkout.`,).toBeVisible({ timeout: UI_TIMEOUT, });
...
async function fillOptionalInput(locator: Locator, value: string,): Promise<void> {
...
async function fillCustomerDetails(page: Page): Promise<void> {
    ...
    await fillRequiredInput(firstNameInput, TEST_DATA.customer.firstName, 'first-name',);

    await fillRequiredInput(lastNameInput, TEST_DATA.customer.lastName, 'last-name',);
    ...
    await fillRequiredInput(postalCodeInput, TEST_DATA.customer.postalCode, 'postal-code',);
...
test.describe('Haarpflege Beauty homepage on iPhone 13 @staging @smoke', () => {
    ...
    test('loads with the expected identity and landmarks', async ({ page, homePage }) => {
        ...
        await test.step('Verify primary page landmarks', async () => {
            await expect(
                homePage.header.root,).toBeVisible({
                    timeout: UI_TIMEOUT,
                });
    ...
    test('searches for a product and fills checkout details', async ({ page, homePage }) => {
        await test.step('Search for a product', async () => {
            const searchInput =
                await getSearchInput(page);
